...
void Server::handleJOIN(Client client, Message message) {
    ...
    if (channel.isLimited() && channel.getLimitUsers() >= channel.getNbClients() && !channel.isRegistered(client.getNickname())) {
    ...
    for (std::vector<Channel&>::iterator it = _channels.begin(); it != _channels.end(); ++it) {
        if (it->getType() & PUBLIC || it->getType() & PRIVATE) {
            if (it->getType & PRIVATE && !it->isInChannel(client.getNickname()))
                res =  ":" + std::string(ADDRESS) +  " 322 " + client.getNickname() + " Prv " + it->getNbClients();
            else
...
void Server::handleKICK(Client client, Message message) {
    ...
    if (!channel.isInChannel(client.getNickname()) || !clientExist(message.parameters[1]) || !channel.isInChannel(getClient(message.parameters[1]).getNickname())) {
    ...
    std::string res = ":" + std::string(ADDRESS) + " KICK " + message.parameters[0] + " " + message.parameters[1];
    std::string kick = ":" + client.getNickname() + "!" + client.getUsername() + "@" + client.getHostname() + " KICK " + message.parameters[0] + " " + message.parameters[1];
...
void Server::handleINVITE(Client client, Message message) {
    ...
    std::string res = ":" + std::string(ADDRESS) + " 341 " + client.getNickname() + " " + message.parameters[1] + " " + client.getNickname();
    std::string invite = ":" + std::string(ADDRESS) + "PRIVMSG " + message.parameters[0] + " :You have been invited to " + message.parameters[1];
...
void Server::handleTOPIC(Client client, Message message) {
    if (message.parameters.size() == 2) {
        if (getChannel(message.parameters[0]).isOperator(client.getNickname())) {
            getChannel(message.parameters[0]).setTopic(message.parameters[1]);
            std::string res = message.prefix + " TOPIC " + message.parameters[0] + " :" + message.parameters[1];
            getChannel(message.parameters[0]).sendChannel(res);
        }
        else
            //not operator
    }
